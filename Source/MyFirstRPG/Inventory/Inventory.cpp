...
void UInventory::CreateSlots()
{
    ...
    {
        // 슬롯 생성
        UInventorySlot* InventorySlot = CreateWidget<UInventorySlot>(GetWorld(), BP_InventorySlot);
...
void UInventory::AddItem(const FItemInfo& NewItemInfo)
{
	for (int i = 0; i < RowSize * ColSize; i++)
	{
        ...
        {
            UInventorySlot *TargetSlot = InventorySlots[i];
...
{
    // 슬롯의 아이템 개수 1 감소
    UInventorySlot* TargetSlot = InventorySlots[SlotIndex];
    TargetSlot->AddItemCount(-1);
